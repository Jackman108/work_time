...
export default function PayrollPage() {
  ...
  const { items: payments, loading, editingItem, setEditingItem, handleAdd, handleUpdate, handleDelete, confirmDialog } = usePageData<ProjectPayment>({
    loadData: getProjectPayments,
    createItem: createProjectPayment,
    updateItem: updateProjectPayment,
    deleteItem: deleteProjectPayment,
    messages: {
      createSuccess: 'Платёж успешно создан',
      updateSuccess: 'Платёж успешно обновлён',
      deleteSuccess: 'Платёж успешно удалён',
      deleteConfirmTitle: 'Удаление платежа',
      deleteConfirmMessage: 'Вы уверены, что хотите удалить этот платёж?'
    }
  });
  ...
  const validateForm = (): boolean => {
    ...
    const requiredValidation = FormValidator.validateRequired(formData, ['project_id', 'date', 'amount']);
  ...
  return (
    <div className="container mt-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1>Поступления на проекты</h1>
        <button className="btn btn-primary" onClick={() => { resetForm(); setShowForm(true); }} disabled={loading}>
          + Добавить платёж
        </button>
      </div>

      <PageLoadingSpinner loading={loading} itemsCount={payments.length} />

      {showForm && (
        <div className="card mb-4">
          <div className="card-header">
            <h5 className="mb-0">{editingItem ? 'Редактирование платежа' : 'Новый платёж'}</h5>
          </div>
      ...
      {!loading && payments.length === 0 && !showForm && (
        <div className="alert alert-info">Платежи не найдены. Создайте первый платёж.</div>
      )}
                ...
                <tbody>
                  {payments.map(payment => (
                    <tr key={payment.id}>
                      ...
                      <td>{payment.notes || '-'}</td>
                      <td>
                        <button className="btn btn-sm btn-outline-primary me-2" onClick={() => setEditingItem(payment)}>Редактировать</button>
                        <button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(payment.id)}>Удалить</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
