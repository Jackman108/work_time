...
export default function CostsPieChart({ data }: CostsPieChartProps) {
  ...
  const chartData = data.map(project => ({
    name: project.name,
    value: project.total_costs
  })).filter(item => item.value > 0);

  if (chartData.length === 0) {
    return <div className="alert alert-info">Нет расходов для отображения</div>;
  }

  return (
    <div className="card mb-4">
      <div className="card-header">
        <h6 className="mb-0">📊 Расходы по объектам</h6>
      </div>
